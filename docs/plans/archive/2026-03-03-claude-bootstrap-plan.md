> **SUPERSEDED — historical record, do not act on this document.**
> Written 2026-03-03 and overtaken by the 2026-09-21 audit. Its marketplace
> and plugin counts, its `install/claude.sh` path, its directory inventory and
> its claim that the `curl | sh` finding was resolved are all false today.
> For what the bootstrap actually does, read `scripts/install_claude.sh`,
